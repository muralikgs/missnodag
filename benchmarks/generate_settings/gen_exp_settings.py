...
exp_specific_params = {
    ...
    "soft-interventions": {"soft-intervention": True},
    "missing-mech-misspec": {"missing-mech": "violate"}
}
...
def main(benchmark_root):
    ...
    for abl_type in exp_types:
        ...
        if abl_type == "ablation-interventions":
            for val in vals: 
            ...
            setting_dir = os.path.join(abl_dir, setting+f"-{vals}")
            cfg["setting"] = setting + f"-{vals}"
            if not os.path.exists(setting_dir):
                os.makedirs(setting_dir)
            
            cfg["data"][setting] = vals
